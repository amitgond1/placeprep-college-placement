  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in pb-20 md:pb-0">
      ...
      <div className="card text-center">
        <div className="flex items-center justify-center gap-8 flex-wrap">
          <div>
            <div
              ...
              style={{ color: accuracy >= 70 ? '#22c55e' : accuracy >= 50 ? '#f59e0b' : '#ef4444' }}
            >
        ...
        {cutoffInfo && (
          <div className={`mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold ${meetsCutoff ? 'bg-green-500/15 text-green-600' : 'bg-red-500/15 text-red-500'}`}>
            {meetsCutoff ? <CheckCircle2 size={16} /> : <XCircle size={16} />}
            {cutoffInfo.label} cutoff ~{cutoffInfo.cutoff}% - You scored {accuracy}% {meetsCutoff ? 'PASS' : 'TRY AGAIN'}
          </div>
        )}
      </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {weakTopics.map((t) => (
                  <span key={t} className="px-3 py-1 rounded-full text-xs font-medium" style={{ background: 'rgba(239,68,68,0.15)', color: '#ef4444' }}>
                    {t} - Practice Now
                  </span>
                ))}
              </div>
      ...
      <div className="card">
        ...
        <div className="space-y-2">
          {result.answers?.map((a, i) => {
            ...
            if (!q) return null;
            return (
              <div key={i} className="rounded-xl border overflow-hidden" style={{ borderColor: a.isCorrect ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)' }}>
                <button className="w-full flex items-center gap-3 p-4 text-left" onClick={() => setExpanded(expanded === i ? null : i)}>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-xs font-bold ${a.isCorrect ? 'text-green-500' : 'text-red-500'}`}>{a.isCorrect ? '?' : '?'}</span>
                    <ChevronDown size={14} className={`transition-transform ${expanded === i ? 'rotate-180' : ''}`} style={{ color: 'var(--text-secondary)' }} />
                  </div>
                </button>

                {expanded === i && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pb-4 space-y-3 border-t" style={{ borderColor: 'var(--border)' }}>
